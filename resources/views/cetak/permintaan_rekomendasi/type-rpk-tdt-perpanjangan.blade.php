...
<table class=MsoTableGrid border=0 cellspacing=0 cellpadding=0 width="100%"
 ...
 <tr>
  ...
  Surat Direktur {{$profile->perusahaan}} Nomor @if($pemohon->no_surat_permohonan != null){{$pemohon->no_surat_permohonan}}@else [DRAFT_NO_SURAT]@endif tanggal @if($pemohon->tgl_surat_permohonan != null){{ \Carbon\Carbon::parse($pemohon->tgl_surat_permohonan)->isoFormat('D MMMM Y') }}@else[DRAFT_TGL_SURAT]@endif perihal Permohonan Persetujuan Perpanjangan Trayek Tetap dan Teratur Angkutan Laut Dalam Negeri, bersama ini kami sampaikan hal-hal
...
<table class=MsoTableGrid border=0 cellspacing=0 cellpadding=0 width="100%"
 ...
 <tr>
  ...
  @if(in_array($pemohon->status_permohonan_id, [7,8,9,10,11,12]))
