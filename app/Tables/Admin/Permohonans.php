...
class Permohonans extends AbstractTable
{
    ...
    public function for()
    {
        ...
        $status_permohonanFilter = AllowedFilter::callback('status_permohonan_filter', function ($query, $value) {
            $query->where(function ($query) use ($value) {
                Collection::wrap($value)->each(function ($value) use ($query) {
                    $query->where('status_permohonan_id', $value);
                });
            });
        });

        return QueryBuilder::for(Permohonan::class)
            ->whereNotIn('perizinan_id', [1, 2, 3])
            ->defaultSort('id')
    ...
    public function configure(SpladeTable $table)
    {
        $table
            ...
            ->column(key: 'profile.alamat', sortable: true, label: 'Alamat')
            ...
            ->selectFilter(key: 'status_permohonan_filter', options: ['12' => 'Selesai'], label: 'Status Izin', noFilterOptionLabel: '-');
